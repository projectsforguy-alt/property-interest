...
export default function DemandCheckForm() {
  ...
  const [postcodeData, setPostcodeData] = useState<{ postcode: string; district: string; admin_district: string } | null>(null);
  const [result, setResult] = useState<DemandResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  function handlePostcodeChange(value: string, data?: { postcode: string; district: string; admin_district: string }) {
    setPostcode(value);
  ...
  async function handleCheck(e: React.FormEvent) {
    ...
    try {
      ...
      setResult({ ...data, postcode: postcodeData.postcode, district: postcodeData.district, admin_district: postcodeData.admin_district });
    } catch {
